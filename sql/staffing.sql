...
(
   id                   int not null auto_increment comment '总请假编号',
   empno                varchar(18) not null comment '员工编号',
   reason               varchar(255) not null comment '请假原因',
   start_date_time      datetime not null comment '假期开始时间',
   end_date_time        datetime not null comment '假期结束时间',
   input_time           datetime not null comment '申请日期',
...
alter table employee add constraint FK_Reference_12 foreign key (deptno)
      references post (deptno) on delete restrict on update restrict;
